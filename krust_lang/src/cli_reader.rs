...
pub fn read_command_line() -> (Option<CLIInfo>, Vec<Log>) {
    ...
    let mut ptr_size: u16 = min(usize::BITS, 2047)
        .try_into()
        .expect("should be valid as max value is less than u16::MAX");
    ...
    for arg in input {
        if arg.ends_with(".txt") && !multiple_file_error {
            if file_path.is_none() {
                ...
                logs.push(Log {
                    log_type: LogType::Error(ErrorType::CLIMultipleFiles),
                    line_and_col: None,
                });
                file_path = None;
...
fn get_args<'a>() -> Result<Vec<String>, Vec<Log>> {
    ...
    if input.is_err() {
        return Err(vec![Log {
            log_type: LogType::Error(ErrorType::CLICantReadArgs),
            line_and_col: None,
        }]);
    }
...
fn handle_ptr_size(arg: &String, logs: &mut Vec<Log>, mut ptr_size: u16) -> u16 {
    ...
    if !arg.starts_with('=') {
        ...
        if ptr_size >= 2048 {
            logs.push(Log {
                log_type: LogType::Error(ErrorType::CLIRequiresNumArgLessThanU16(
                    COMPILER_FLAGS[0].to_string(),
                    2048,
                )),
                line_and_col: None,
        ...
        if ptr_size < 8 {
            logs.push(Log {
                log_type: LogType::Error(ErrorType::CLIRequiresNumArgAtLeastU16(
                    COMPILER_FLAGS[0].to_string(),
                    8,
                )),
                line_and_col: None,
...
fn get_file_size(
    ...
    if let Some(ref path) = file_path {
        ...
        logs.push(Log {
            log_type: LogType::Error(ErrorType::CLINoFile),
            line_and_col: None,
        });
    }
    file_size
...
fn handle_compiler_flag_issues(
    ...
    if let Some(file_path) = file_path {
        ...
        if ptr_size % 8 != 0 {
            logs.push(Log {
                log_type: LogType::Warning(WarningType::CLIArgRoundedDownU16(
                    COMPILER_FLAGS[0].to_string(),
                    8,
                )),
                line_and_col: None,
        ...
        {
            logs.push(Log {
                log_type: LogType::Warning(WarningType::CLITargetLargerThanMachine(ptr_size)),
                line_and_col: None,
            });
        } else if ptr_size
            < usize::BITS
        ...
        {
            logs.push(Log {
                log_type: LogType::Error(ErrorType::CLIFileToBig(ptr_size)),
                line_and_col: None,
            });
            return (
                Some(CLIInfo {
                    file_path: file_path.to_string(),
                    cli_args: [ptr_size_bytes, detailed_err],
                }),
                logs.clone(),
            );
        }
